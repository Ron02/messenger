...
//retruns the name of the user from the msg
std::string Helper :: getName(std::string msg) 
{
	std::string::size_type sz;
	std::string len = msg.substr(3, 2);
	int iLen = stoi(len, &sz);
	return msg.substr(5, iLen);
}

//retruns string of all the users with & seperate
std::string Helper :: getAllUsers(std::set <std::string> vec) 
{
	std::string allUsers = "";
	for (auto i : vec) 
	{
	...
	return allUsers.substr(0, allUsers.length() - 1);
}